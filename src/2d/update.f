...
c
 ...
 101          format(' updating pt. ',2i4,' of grid ',i3,' using ',2i4,
     1               ' of grid ',i4)
              write(outunit,102)(alloc(iadd(ivar,i,j,loc,nvar,mitot)),
     .          ivar=1,nvar)
...
c
           do 35 ivar = 1, nvar
 35           alloc(iadd(ivar,i,j,loc,nvar,mitot)) = 0.d0
c
           if (mcapa .eq. 0) then
     ...
     1             alloc(iadd(ivar,i,j,loc,nvar,mitot)) + 
     2             alloc(iaddf(ivar,iff+ico-1,jff+jco-1,locf,nvar,mi))
 40              continue
 50            continue
            do 60 ivar = 1, nvar
 60          alloc(iadd(ivar,i,j,loc,nvar,mitot)) = 
     .          alloc(iadd(ivar,i,j,loc,nvar,mitot))/totrat
     ...
     .          locfaux,mcapa,naux,mi))
               do 41 ivar = 1, nvar
                 alloc(iadd(ivar,i,j,loc,nvar,mitot))= 
     1              alloc(iadd(ivar,i,j,loc,nvar,mitot)) + 
     2              alloc(iaddf(ivar,iff+ico-1,
     3                  jff+jco-1,locf,nvar,mi))*capa
 41              continue
 51            continue
            do 61 ivar = 1, nvar
 61          alloc(iadd(ivar,i,j,loc,nvar,mitot)) = 
     1          alloc(iadd(ivar,i,j,loc,nvar,mitot))/
     2          (totrat*alloc(iaddcaux(i,j,loccaux,mcapa,naux,mitot)))
           endif
c
